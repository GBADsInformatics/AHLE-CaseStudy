...
flask_server, app = fa.instantiate_app(app_title, external_scripts, external_stylesheets) # dont change name "flask_server".  Gunicorn expects to find it
...
ecs_species_options2 = [{'label': i, 'value': i, 'disabled': False} for i in ["All",
                                                                             "All Small Ruminants",
                                                                             "All Poultry",
                                                                             "Cattle"
                                                                             ]]
...
def prep_ahle_forwaterfall_ecs(INPUT_DF, ITEM_LIST):
   ...
   plot_items_upper = [i.upper() for i in ITEM_LIST]
   ecs_ahle_waterfall = ecs_ahle_waterfall.loc[ecs_ahle_waterfall['item'].str.upper().isin(plot_items_upper)]

   # Sort data by Item column according to order specified
   ecs_ahle_waterfall['item'] = ecs_ahle_waterfall['item'].astype('category')
   ecs_ahle_waterfall.item.cat.set_categories(ITEM_LIST, inplace=True)
   ecs_ahle_waterfall = ecs_ahle_waterfall.sort_values(["item"])

   # Rename some items to be more descriptive
...
gbadsDash.layout = html.Div([
    ...
    dbc.Tabs([
                                 ...
                                 dbc.Card([
                                     dbc.CardBody([
                                         ...
                                         dbc.Row([
                                             # Top Level
                                             dbc.Col([
                                                 html.H6("Top Level", id="select-top-lvl-attr-ecs-title", style=control_heading_style),
                                        ...
                                        dbc.Row([
                                             # Drilldown 1
                                             dbc.Col([
                                                 html.H6("Drilldown 1", id="select-dd-1-attr-ecs-title", style=control_heading_style),
                                            ...
                                            # Drilldown 2
                                            dbc.Col([
                                                html.H6("Drilldown 2", id="select-dd-2-attr-ecs-title", style=control_heading_style),
                                         ...
                                         dbc.Row([
                                             # Drilldown 3
                                             dbc.Col([
                                                 html.H6("Drilldown 3", id="select-dd-3-attr-ecs-title", style=control_heading_style),
                                             ...
                                             # Drilldown 4
                                             dbc.Col([
                                                 html.H6("Drilldown 4", id="select-dd-4-attr-ecs-title", style=control_heading_style),
        ...
        #### INFORMATICS ETHIOPIA LIVESTOCK POPULATION DASHBOARD
        dbc.Tab(label="Population",
                tabClassName="flex-grow-1 text-center",
                    tab_style = tab_style,
                    style = {"height":"100vh",
                        },
                children =[
            html.Iframe(src="https://gbadske.org/dashboards/population/",
                        style={"width":"100%",
                            "height":"1000px",
                                },)
        ### END OF INFORMATICS ETHIOPIA LIVESTOCK POPULATION DASHBOARD TAB
            ]),
...
# Year selector for attribution
@gbadsDash.callback(
    Output('select-year-ecs-title2','children'),
    Output('select-year-ecs2','options'),
    Output('select-year-ecs2','value'),
...
# End year selector (only available if over time display on AHLE and Attribution tabs)
@gbadsDash.callback(
    Output('select-end-year-ecs-title2','children'),
    Output('select-end-year-ecs-title2','style'),
    Output('select-end-year-ecs2','options'),
    Output('select-end-year-ecs2','value'),
    Output('select-end-year-ecs2','style'),
    Input('select-attr-display-ecs','value'),
    Input('select-species-ecs','value'),
    )
...
def update_dd1_options_ecs(graph, top_lvl_hierarchy):
    ...
    if graph == 'Bar':
        ...
        for d in options:
            if d['value'] == top_lvl_hierarchy:
                ...
                d['disabled']=False
        display_val_style = {'display': 'block'}
    ...
    return options, value, display_val_style, display_title_style
...
def update_ahle_graph_controls(graph, geo_view):
    ...
    return block_vals, block_title, value, options2
...
def update_ecs_ahle_data(currency, species, prodsys):
    ...
    input_df=input_df.loc[(input_df['group'] == 'Overall')]
...
#### -- Figures
# ------------------------------------------------------------------------------
# AHLE Bar chart, Waterfall, or Longitudinal Graph
@gbadsDash.callback(
...
def update_ahle_chart_ecs(
    ...
    # Prep the data
    prep_df = prep_ahle_forwaterfall_ecs(input_df, bar_chart_plot_items)

    # Get list of items from data - may be subset of list specified because not all items appear for all age/sex groups
    bar_chart_plot_items_indata = list(prep_df['item'].unique())
            ...
            if compare == 'Ideal':
                ...
                ecs_waterfall_fig.update_layout(
                    xaxis = dict(
                        tickmode = 'array',
                        tickvals = x_len,
                        ticktext = bar_chart_plot_items_indata
                    )
                )
